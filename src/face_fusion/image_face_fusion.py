...
from .peppa_pig_face.facer import FaceAna
...
class ImageFaceFusion():

    def __init__(self, model_dir: str,model_file='pytorch_model.pt', *args, **kwargs):
    ...
    def inference(self, template_path, user_path):
        if isinstance(template_path,str):
            template=cv2.imread(template_path)
            template_img = cv2.cvtColor(template, cv2.COLOR_BGR2RGB) #bgr转rgb
        elif isinstance(template_path,np.ndarray):
            template_img=template_path
        if isinstance(user_path,str):
            user=cv2.imread(user_path)
            user_img = cv2.cvtColor(user, cv2.COLOR_BGR2RGB) #bgr转rgb
        elif isinstance(user_path,np.ndarray):
            user_img=user_path
