...
def get_web_ext_dir():
    config = get_extension_config()
    name = config["name"]
    dir = get_comfy_dir("web/extensions/lam")
...
def get_extension_config(reload=False):
    ...
    config_path = get_ext_dir("lam.json")
    if not os.path.exists(config_path):
        log("Missing lam.json, this extension may not work correctly. Please reinstall the extension.",
